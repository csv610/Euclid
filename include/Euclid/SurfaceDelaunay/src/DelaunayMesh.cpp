                ...
                std::vector<bool>& inqueue)
{
    ...
    inqueue.assign(num_edges(mesh), false);
...
{
    auto eimap = get(boost::edge_index, mesh);
    physical.assign(num_edges(mesh), true);
    for (auto e : edges(mesh)) {
           ...
           std::unordered_set<vertex_t<Mesh>>& split_vertices)
{
    ...
    physical_edges.resize(num_edges(mesh), true);
    ...
    queue.push(esu);
    queue.push(esv);
    inqueue[get(eimap, esp)] = true;
    inqueue[get(eimap, esq)] = true;
    inqueue[get(eimap, esu)] = true;
    inqueue[get(eimap, esv)] = true;
    physical_edges[get(eimap, esp)] = true;
    physical_edges[get(eimap, esq)] = true;
    physical_edges[get(eimap, esv)] = false;
    physical_edges[get(eimap, esu)] = false;
